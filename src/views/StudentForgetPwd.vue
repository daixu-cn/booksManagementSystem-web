<template>
  <div class="container">
    ...
    <el-input v-if="isStep1Complete" placeholder="请输入对应的个人学号" v-model="id" :disabled="isDisabled">
...
export default {
  ...
  methods: {
    async next () {
      ...
      if (this.active === 0) {
        const url = '/admin/borrow/StudentForgetPwd/'
        const params = {
          params: {
            stu_id: this.id,
            stu_name: this.name
        ...
        await axios.post(url, params).then(res => {
          if (res.data.code === 200) {
            ...
            this.$message.error('学号与姓名不匹配,请检查后重新输入')
          }
        })
      }
      ...
      if (this.active === 1) {
        if (this.formLabelAlign.pwd === '') {
          ...
          const url2 = '/admin/borrow/StudentUpdatePwd/'
          const params2 = {
            params: {
              stu_id: this.id,
              stu_name: this.name,
